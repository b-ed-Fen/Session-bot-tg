assembly = {
    ...
    'greeting': {
        'us': 'Welcome! Adjust everything to your liking: /settings. '
              '\nIf something went wrong or something is not clear to you, look at: /help.',
        'ua': 'Ласкаво просимо! Налаштуйте все за своїм смаком: /settings. '
              '\nЯкщо щось пішло не так або вам щось не ясно подивіться: /help.',
        'ru': 'Добро пожаловать! Настройте все по своему вкусу: /settings. '
              '\nЕсли что-то пошло не так или вам что-то не ясно посмотрите: /help. '
    ...
    'help': {
        'us': 'How to create your own schedule? '
              ...
              'file, enter /template.\n'
              'How to configure a bot for yourself?'
              ...
              '\nIs the problem still not resolved? Email me: @ihelp_u',
        ...
        'ua': 'Як створити свій розклад? '
              ...
              '\nПроблема все ще не вирішена? Напиши мені: @ihelp_u',
        ...
        'ru': 'Как создать своё расписание?'
              ...
              '\nПроблема все еще не решена? Напиши мне: @ihelp_u'
    },
    ...
    'ed down': {
        'us': 'There is nowhere below',
        'ua': 'Нижче нікуди',
        'ru': 'Ниже некуда'
    },

    'ed up': {
        'us': 'There is nothing above',
        'ua': 'Вище нічого немає',
        'ru': 'Выше ничего нет'
    },

    'Time instead of number': {
        'us': 'Time instead of class number: ',
        'ua': 'Час замість номера заняття: ',
        'ru': 'Время вместо номера занятия: '
    },

    'work day edit': {
        'us': 'What days should I send you notifications?',
        'ua': 'У які дні я повинен відправляти вам повідомлення?',
        'ru': 'В какие дни я должен отправлять вам уведомления?'
    },

    'letime': {
        'us': 'To change the call schedule, write a message describing the beginning of each lesson in H:M format '
              '(for example, 8:30). '
              '\nThe start time of the lesson must be exactly one line. '
              '\nThe call schedule currently consists of:',
        'ua': 'Для зміни розкладу дзвінків напишіть повідомлення, в якому опишіть початок кожного уроку в форматі '
              '«Г:Х» (наприклад, 8:30). '
              '\nЧас початку заняття повинно займати рівно один рядок. '
              '\nРозклад дзвінків на даний момент складається з:',
        'ru': 'Для изменения расписания звонков напишите сообщение, в котором опишите начало каждого урока в формате '
              '«Ч:М» (например, 8:30). '
              '\nВремя начала занятия должно занимать ровно одну строку. '
              '\nРасписание звонков на данный момент состоит из: '
    }
}
...
def settings(client):
    user_settings = {
        'us': 'Settings: \n'
              '/merger - The merger of weeks = ' + ((assembly['enable']['on'][client.settings['language']]) if (
        client.settings['combination of weeks']) else (assembly['enable']['off'][client.settings['language']])) + '\n'
                                                                                                                  '/language - language selection = ' + str(
            client.settings['language']) + '\n'
                                           '/UTC - Set your time at UTC = ' + str(client.settings['UTC']) + '\n'
                                                                                                            '/inverting - Swap the weekly schedule = ' + (
                  (assembly['enable']['on'][client.settings['language']]) if (client.position['week even']) else (
                  assembly['enable']['off'][client.settings['language']])) + '\n'
                                                                             '/notification - Turn on notifications = ' + (
                  (assembly['enable']['on'][client.settings['language']]) if (client.settings['notification']) else (
                  assembly['enable']['off'][client.settings['language']])) + '\n'
                                                                             '/time - Set the time for seeing = ' + str(
            client.time) + '\n'
                           '/torn - Time instead of numbers in the schedule = ' + (
                  (assembly['enable']['on'][client.settings['language']]) if (
                  client.settings['Time instead of number']) else (
                  assembly['enable']['off'][client.settings['language']])) + '\n'
                                                                             ...
                                                                             '/letime - Set a time for each lesson',

        'ua': 'Налаштування: \n'
              '/merger - Злиття тижнів = ' + ((assembly['enable']['on'][client.settings['language']]) if (
        client.settings['combination of weeks']) else (assembly['enable']['off'][client.settings['language']])) + '\n'
                                                                                                                  '/language - Вибір мови = ' + str(
            client.settings['language']) + '\n'
                                           '/UTC - Ваш час по UTC = ' + str(client.settings['UTC']) + '\n'
                                                                                                      '/inverting - Поміняти розклад тижнів = ' + (
                  (assembly['enable']['on'][client.settings['language']]) if (client.position['week even']) else (
                  assembly['enable']['off'][client.settings['language']])) + '\n'
                                                                             '/notification - Повідомлення = ' + (
                  (assembly['enable']['on'][client.settings['language']]) if (client.settings['notification']) else (
                  assembly['enable']['off'][client.settings['language']])) + '\n'
                                                                             '/time - Час повідомлення = ' + str(
            client.time) + '\n'
                           '/torn - Час замість цифр = ' + ((assembly['enable']['on'][client.settings['language']]) if (
        client.settings['Time instead of number']) else (assembly['enable']['off'][client.settings['language']])) + '\n'
                                                                                                                    '/edit -  Коригуйте графік "на ходу"\n'
                                                                                                                    '/doc - Отримати свій графік файлом\n'
                                                                                                                    '/wdn - Виберіть, в які дні ви хочете отримувати повідомлення\n'
                                                                                                                    '/letime - Встановити час для кожного заняття',

        'ru': 'Настройки: \n'
              '/merger - Слияние недель = ' + ((assembly['enable']['on'][client.settings['language']]) if (
        client.settings['combination of weeks']) else (assembly['enable']['off'][client.settings['language']])) + '\n'
                                                                                                                  '/language - Язык интерфейса = ' + str(
            client.settings['language']) + '\n'
                                           '/UTC - Ваше время по UTC = ' + str(client.settings['UTC']) + '\n'
                                                                                                         '/inverting - Поменять местами недели = ' + (
                  (assembly['enable']['on'][client.settings['language']]) if (client.position['week even']) else (
                  assembly['enable']['off'][client.settings['language']])) + '\n'
                                                                             '/notification - Уведомления = ' + (
                  (assembly['enable']['on'][client.settings['language']]) if (client.settings['notification']) else (
                  assembly['enable']['off'][client.settings['language']])) + '\n'
                                                                             '/time - Время уведомлений = ' + str(
            client.time) + '\n'
                           '/torn - Время вместо цифр = ' + (
                  (assembly['enable']['on'][client.settings['language']]) if (
                  client.settings['Time instead of number']) else (
                  assembly['enable']['off'][client.settings['language']])) + '\n'
                                                                             '/edit - Корректировать график "на ходу"\n'
                                                                             '/doc - Получить свой график файлом\n'
                                                                             '/wdn - В какие дни отправлять уведомления\n'
                                                                             '/letime - Установить время для каждого занятия',
    }
